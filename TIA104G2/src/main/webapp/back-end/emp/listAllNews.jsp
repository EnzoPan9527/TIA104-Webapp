...
<table>
	...
	<c:forEach var="newsVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${newsVO.description}</td>
			<td>${newsVO.post_time}</td>
			<td>${newsVO.create_time}</td>
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/emp/news.do" style="margin-bottom: 0px;">
